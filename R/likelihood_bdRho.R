...
#'   \item If a set of phylogenies are used for computing the log likelihood then all the parameters except the one relatives to the hyperparameters of the sampling probabilitie(s) distribution described in this section: \code{div}, \code{turn}, \code{ylamb}, \code{tun.init} and \code{seqphy.init} have to be of length the number of phylogenies used. Note that this function is not able to evaluate negative rates.
#' }
#' @details This function is specifically intended to be used on phylogenies with unknown or highly uncertain global diversity estimates. We thus do not know the sampling probability with accuracy. Note that the sampling probability is never estimated.
#'
