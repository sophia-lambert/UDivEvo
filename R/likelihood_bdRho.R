...
#' @param beta Logical. This argument is only used if \code{yj = NULL} and \code{reparam = FALSE}. If \code{TRUE} a beta distribution is assumed on the sampling probabilitie(s). Note that the parameters of the beta distribution can be fixed or inferred and can be specified in the function generated by the function \code{likelihood_bdRho} using the arguments \code{a} for the parameter \eqn{\alpha} (\eqn{\alpha>0}) and \code{b} for the parameter \eqn{\beta} (\eqn{\beta>0}).
#' @param unif Logical. This argument is only used if \code{yj = NULL} and \code{reparam = FALSE}. If \code{TRUE} a uniform distribution is assumed on the sampling probabilitie(s). Note that the parameters of the uniform distribution can be fixed or inferred and can be specified in the function generated by the function \code{likelihood_bdRho} using the arguments \code{a} for the lower bound of the uniform distribution (\eqn{0\lea<1}) and \code{b} for the higher bound of the uniform distribution (\eqn{0<b\le1} and \eqn{b>a}).
...
#' @param tuned_dichotomy Logical. This argument is only used if \code{yj = NULL} and \code{reparam = FALSE}. If \code{TRUE}, when the log likelihood is equal to non finite value due to approximations, a dichotomy search is performed to find a tuning parameter that will be used for getting a finite value of the log likelihood. If \code{TRUE}, the log likelihood will take longer to calculate. Else if \code{FALSE}, no dichotomy search is performed if the log likelihood is equal to non finite value and the function will thus return this non finite value due to approximations.
...
#'   \item If a unique phylogeny is used, \code{yj = NULL} and \code{reparam = FALSE}, then the \eqn{birth-death∫\rho} model is used and the function created takes \code{tun.init}: the initial tuning parameters values and \code{seqphy.init}: the phylogenies number for which to use the corresponding tuning parameter. This former function creates another function that takes as argument \code{div}: the net diversification rate, \code{turn}: the turnover rate, and hyperparameters of the sampling probabilitie(s) distribution \code{a}: the first argument of the sampling probabilitie(s) distribution chosen (see \code{\link{phi}} for more details) and \code{b}: the second argument of the sampling probabilitie(s) distribution chosen (see \code{\link{phi}} for more details).
...
#' }
#' @details This function is specifically intended to be used on phylogenies with unknown or highly uncertain global diversity estimates thus not knowing the sampling probability with accuracy. Note that the sampling probability is never estimated and that this function is not able to evaluate negative rates.
#'
...
#'   \item{tun.init}{Numeric vector. The initial tuning parameters values. Typically, it will take the value \eqn{\log _{10} 1}. The length of the numeric vector equals the number of phylogenie(s) used.}
